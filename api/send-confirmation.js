...
function getTexts(language = 'es') {
  const texts = {
    es: {
      ...
      paymentReceived: 'Hemos recibido tu pago correctamente',
      processingStarted: 'Tu pedido ha entrado en proceso',
    ...
    en: {
      ...
      paymentReceived: 'We have received your payment correctly',
      processingStarted: 'Your order has entered processing',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const emailContent = `
        <div class="content">
          <h1 class="title">${texts.orderConfirmation}</h1>
          ...
          <div class="info-row">
            ...
            <span class="status processing">${texts.processing.toUpperCase()}</span>
          </div>
          <div class="info-row">
        ...
        <div style="background: #dbeafe; border-radius: 8px; padding: 24px; margin: 20px 0; border-left: 4px solid #2563eb;">
           <h4 style="color: #1e40af; margin-bottom: 16px; font-size: 16px;">🚀 ${texts.processingStarted}</h4>
           <div style="color: #1e40af; font-size: 14px; line-height: 1.6;">
             <p style="margin: 8px 0;"><strong>${texts.estimatedTime}</strong></p>
             <p style="margin: 8px 0;">${texts.keepInformed}</p>
           </div>
         </div>
    ...
    const { data, error } = await resend.emails.send({
      ...
      subject: `${texts.orderConfirmation} #${dbOrderData.id} - ${serviceName}`,
