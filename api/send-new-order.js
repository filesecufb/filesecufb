...
function getTexts(language = 'es') {
  const texts = {
    ...
    en: {
      ...
      notification: 'Notification: We will inform you when it is ready',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('Sending new order notification for order:', orderData.id);
    ...
    const emailContent = `
        <div class="content">
          ...
          <div style="text-align: center; margin: 20px 0; padding: 20px; background: #ecfdf5; border-radius: 8px; border-left: 4px solid #10b981;">
            <p style="color: #065f46; font-size: 16px; margin: 0; font-weight: 600;">
              ¡Hola ${dbOrderData.client_name}! Hemos recibido tu pedido correctamente.
            </p>
          </div>
          ...
          <div class="info-row">
            ...
            <span class="status pending">${texts.pending.toUpperCase()}</span>
          </div>
          <div class="info-row">
        ...
        <div style="background: #f0f9ff; border-radius: 8px; padding: 24px; margin: 20px 0; border-left: 4px solid #2563eb;">
           <h4 style="color: #1e40af; margin-bottom: 16px; font-size: 16px;">${texts.whatNext}</h4>
           <div style="color: #1e40af; font-size: 14px; line-height: 1.6;">
             <p style="margin: 8px 0;">1. <strong>${texts.technicalReview}</strong></p>
             <p style="margin: 8px 0;">2. <strong>${texts.preparation}</strong></p>
             <p style="margin: 8px 0;">3. <strong>${texts.notification}</strong></p>
           </div>
         </div>
        ...
        <div style="background: #f8fafc; border-radius: 8px; padding: 24px; margin: 20px 0; text-align: center;">
           <p style="color: #475569; font-size: 14px; margin: 0; line-height: 1.6;">
             ${texts.questions}<br>
             <strong>${texts.keepInformed}</strong>
           </p>
        </div>
    ...
    if (error) {
      console.error('Error enviando nuevo pedido:', error);
