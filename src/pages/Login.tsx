...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !password) {
      setError(t('auth.login.errors.fieldsRequired'))
    ...
    try {
      ...
      if (error) {
        ...
        // Esperar un poco para que el perfil se actualice
        setTimeout(() => {
      ...
      setError(error.message || t('auth.login.errors.generic'))
    } finally {
      setIsSubmitting(false)
    }
  }
      ...
      <div className="max-w-md w-full space-y-8 relative z-10">
        <div className="bg-gradient-dark backdrop-blur-sm rounded-2xl border border-elegant shadow-elegant p-8">
          ...
          >
            ...
            <span>{isGoogleLoading ? 'Conectando...' : t('auth.login.google')}</span>
          </button>
